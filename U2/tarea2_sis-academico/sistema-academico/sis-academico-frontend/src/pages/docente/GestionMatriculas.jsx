...
const GestionMatriculas = () => {
    ...
    const fetchAsignaturasDocente = async (usuario) => {
        try {
            ...
            const currentUser = usersData.find(u => u.usuario === usuario);
            ...
            if (currentUser) {
                ...
                const asignaturasData = Array.isArray(dataAsig) ? dataAsig : (dataAsig.data || []);

                // 3. Filtrar las mías
                const misAsignaturas = asignaturasData.filter(a => a.usuarioId === currentUser.id);
