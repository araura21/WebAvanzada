...
const MisCompaneros = () => {
    ...
    const fetchMisMatriculas = async () => {
        ...
        try {
            ...
            const matriculasData = Array.isArray(matriculas) ? matriculas : (matriculas.data || []);

            setAsignaturas(matriculasData);

            // Cargar compañeros por cada materia
            const companerosMap = {};

            await Promise.all(matriculasData.map(async (mat) => {
                ...
                try {
                    ...
                    const companerosData = Array.isArray(companeros) ? companeros : (companeros.data || []);
                    // Filtrar para no mostrarme a mi mismo (opcional)
                    companerosMap[asigId] = companerosData.filter(c => c.id != id);
                } catch (e) {
